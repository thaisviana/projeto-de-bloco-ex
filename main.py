...
def mostra_titulo(texto, x, y, tamanho_fonte=18):
...
def formata_valores(soma):
    ...
    for item in infos:
          ...
          mostra_titulo(f'{round(vms,2)} MB',400, 160 + soma_indices*22)
          mostra_titulo(f'{round(rss,2)} MB',500, 160 + soma_indices*22)
          mostra_titulo(f'{round((vms/(soma_vms*1024))*100,2)} %',620, 160 + soma_indices*22)
          mostra_titulo(f'{round((rss/(soma_rss*1024))*100,2)} %',700, 160 + soma_indices*22)
...
def cria_abas():
    ...
    for i in range(0,4):
        ...
        mostra_titulo_aba(f"{titulo[i]}", (largura_tela/4 * aba_type)+100)
    return lista_de_abas

def mostra_conteudo_aba_0(conta_segundos):
    mostra_titulo(f'ARQUIVOS - {conta_segundos}s',100, 100, 24)
    mostra_titulo(f'index',40,160)
    mostra_titulo(f'Nome Arquivo',100,160)
    mostra_titulo(f'Tamanho',240,160)
    mostra_titulo(f'Data Modificação',340,160)
    mostra_titulo(f'Data Criação',590,160)
    lista = os.listdir()
    soma_indices = 1
    for i in lista: 
        if os.path.isfile(i):
            mostra_titulo(f'{soma_indices}',40, 160 + soma_indices*22)
            mostra_titulo(f'{i}', 100, 160 + soma_indices*22)
            mostra_titulo(f'{round(os.stat(i).st_size/1024,2) }KB',240, 160 + soma_indices*22)
            mostra_titulo(f'{time.ctime(os.stat(i).st_mtime)}',340, 160 + soma_indices*22)
            mostra_titulo(f'{time.ctime(os.stat(i).st_atime)}',590, 160 + soma_indices*22)
            soma_indices += 1

def mostra_conteudo_aba_1(conta_segundos):
    mostra_titulo(f'PROCESSOS - {conta_segundos}s',100, 100, 24)
    mostra_titulo(f'PID',100,160)
    mostra_titulo(f'NOME',160,160)
    mostra_titulo(f'VMS',400,160)
    mostra_titulo(f'RSS',500,160) 
    mostra_titulo(f'% VMS',620,160)
    mostra_titulo(f'% RSS',700,160) 
